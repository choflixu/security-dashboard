...
export default function App() {
  ...
  const [results, setResults]         = useState({})   // scriptId -> ScriptResult
  const [running, setRunning]         = useState({})   // scriptId -> bool
  const [activeOutput, setActive]     = useState(null) // scriptId shown in terminal
  ...
  useEffect(() => {
    fetchScripts()
      .then(async (data) => {
        setScripts(data)
        // Load last known results in parallel
        const resultsMap = {}
        await Promise.allSettled(
          data.map(async (s) => {
            const r = await fetchLastResult(s.id).catch(() => null)
            if (r) resultsMap[s.id] = r
          })
        )
      ...
      .catch((err) => {
        setError('No se puede conectar con el backend. ¿Está corriendo Spring Boot?')
        setLoading(false)
        console.error(err)
      })
  ...
  // ── Handle WS messages for a script ─────────────────────────────────────
      ...
      setResults(prev => {
        ...
        return {
          ...
          [scriptId]: {
            ...
            status: 'RUNNING',
            output: (current?.output || '') + (msg.output || ''),
          }
        }
      })
    } else {
      // Final result
      setResults(prev => ({ ...prev, [scriptId]: msg }))
  ...
  const handleRun = useCallback(async (scriptId) => {
    ...
    setResults(prev => ({ ...prev, [scriptId]: { scriptId, status: 'RUNNING', output: '' } }))
    setRunning(prev => ({ ...prev, [scriptId]: true }))
    setActive(scriptId)

    // Subscribe to WS before firing
  ...
  // ── Summary counts ────────────────────────────────────────────────────────
  const counts = { success: 0, warning: 0, error: 0, running: 0 }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          ...
          <div>
            ...
            <div style={{
              ...
              {wsReady ? 'WebSocket conectado' : 'Conectando...'}
            </div>
          </div>
